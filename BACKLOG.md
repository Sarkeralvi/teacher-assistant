...
- **Current status update — 2026-08-22 (supersedes older local-AI status notes below):** PaddleOCR is retired from active workflows; RapidOCR is tier-1 for printed reference pages, while Qwen3.8 performs non-thinking visual mapping/transcription for handwritten script evidence. The active engineering now includes calibrated escalation foundations, a page-mapping safety net, mandatory fail-closed Qwen3.6/Qwen3.8 inference leases, the 20-case visual-transcription evaluator with a valid OCR-quality `NO_GO` branch, and isolated Qwen3.6-versus-Qwen3.8 text-grading bake-off/replay tooling. Every local inference requires the matching live lease; visual text is teacher-confirmed evidence only; text grading receives no student image; suggestions remain review-required; zero retries/fallbacks; normal cohort grading stays disabled. **Pilot status: blocked** pending a teacher-signed live 20-case `PASS`, the two real 18-call grading candidate runs plus signed comparison, one live tiered reference extraction, and one complete teacher-supervised rehearsal. Current handwriting fixture diversity remains too small for a classroom-wide threshold claim. Semi/Fully Automated remain disabled.
