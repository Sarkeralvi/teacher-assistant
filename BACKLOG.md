...
- **Current status update — 2026-08-25 (supersedes older local-AI status notes below):** the active rescue architecture restores PaddleOCR-VL-1.6 with PP-DocLayoutV3 as the primary page/block OCR phase, followed by Qwen3.6 for reference correlation, block-ID mapping, and text-only draft grading. Qwen3.8 is restricted to an explicit non-thinking visual-transcription rescue after the teacher rejects a direct Paddle draft. RapidOCR/Tesseract and the former Qwen3.8-first paths remain diagnostic or historical only and are not active teacher actions. PaddleOCR, Qwen3.6, and Qwen3.8 are exclusive database-leased GPU phases; every inference fails before HTTP without the matching live lease. Mapping, exact transcript confirmation, and full-answer image confirmation are separate teacher gates; suggestions remain draft-only; retries/fallbacks and normal cohort grading remain disabled. A clean bounded host acceptance run passed one Paddle OCR call, three Qwen3.6 calls covering reference correlation, answer mapping, and text-only draft grading, and one non-thinking Qwen3.8 rescue call, with models loaded sequentially and unloaded afterward. **Pilot status: blocked** pending a teacher-signed curated quality `PASS` for the rescued workflow and one signed end-to-end supervised rehearsal. Semi/Fully Automated remain disabled.
