    ...
    def read_page(
        ...
        *,
        ...
        prompt = (
            ...
            "label_source is required on every block and must be exactly one of three values. "
            "Use heading when a visible handwritten question heading on this page identifies "
            "the block. Use continuation for every block whose question_label is null, meaning "
            "it carries on the labelled block above it or an open continuation from the "
            "previous page. Use inferred when the block belongs to a labelled question but no "
            "heading for it is visible on this page, so the label came from context; inferred "
            "blocks go to teacher review and cannot auto-pass. Never omit label_source and "
            "never return null for it. Preserve crossed-out and overwritten work verbatim with "
