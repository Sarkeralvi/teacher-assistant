...
Status date: 2026-08-17. This section is the canonical status for local OCR/Qwen and cohort safety; older task notes remain historical records.
...
- **OCR evidence:** persisted direct baselines remain non-canonical drafts. `math_handwriting_rescue_v2` uses PP-OCRv6 geometry, missing-line image projection, PaddleOCR-VL band/formula context, and explicitly labeled cross-engine visual alternatives. Teachers select immutable candidates without cropping or typing; confirmation still does not confirm full-answer coverage or create a grade.
...
- **Verification completed:** 397 backend tests pass (2 skipped), Ruff passes, and migration head is `0022`. Live v2 OCR runs on the supplied submission `#39` produced selectable candidates preserving its critical complements, intersections, fractions, written method, final `7/10`, and the complete `1(a)(ii)` calculation. These engineering runs did not confirm evidence or call Qwen. The supplied three-PDF reference bundle and separate local-Qwen structured grading smoke remain successful.
- **Verification remaining:** a teacher must visually select and confirm the new v2 candidates, separately confirm full-answer coverage, rehearse local-Qwen grading from that confirmed transcript, and complete a fresh curated run with all mandatory sign-offs. Real cohort grading remains locked until a fresh signed run passes with no severe stop condition.
