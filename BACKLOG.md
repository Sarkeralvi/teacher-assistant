...
- **Status correction — 2026-09-08 (code reading at commit `29502c5`; supersedes the "OCR stage is not rewired" clause in the 2026-08-20 historical entry below):** the curated gate's OCR stage **is** rewired to Qwen3.8 visual transcription. Evidence in `apps/api/packages/evaluation/local_curated_evaluation.py`: `PROTOCOL_VERSION = "local-curated-qwen38-vision-v2"` (`:28`); `OcrCaseResult` pins `provider="llama_cpp_qwen38"`, `model="qwen3.8-27b-q4km"`, `profile="qwen38_verbatim_visual"`, `reasoning_mode="off"` (`:302-335`); `run_ocr_stage` (`:2559`); and `docs/LOCAL_CURATED_EVAL_RUNBOOK.md` §3 documents the live `run-ocr` invocation. **Rewired is not runnable, and runnability was NOT verified:** `tests/test_local_curated_evaluation_integration.py` errored at setup with `sqlalchemy.exc.OperationalError: (psycopg.errors.ConnectionTimeout) connection timeout expired` because the pilot stack was not running and starting it was not authorized; no `prepare` or `run-ocr` was executed. **No result may still be cited from this gate.** Separately recorded, unresolved: `docs/LOCAL_CURATED_EVAL_RUNBOOK.md` §4 instructs `run-grading --expected-model qwen3.8-27b-q4km`, which `local_curated_evaluation.py:3651` refuses because `SUPPORTED_GRADING_MODELS` is `("qwen3.6-35b-a3b-q4km",)` (`:33`). Whether the runbook or the guard is wrong was not adjudicated — see `docs/FOUNDER_PILOT_REHEARSAL_READINESS.md` §6. As documented, the gate would hard-fail after ground-truth signing and 20 spent visual calls.
...
- Scope: create a concise teacher-facing guide for the allowed Custom Controlled V0 flow: manual setup, manual answer evidence text, readiness gate, single-packet draft grading, teacher review/approval, approved-only export, troubleshooting, and stop conditions.
- Files affected: `docs/CUSTOM_CONTROLLED_V0_TEACHER_QUICK_START.md`, `docs/README.md`, `BACKLOG.md`
