...
def test_local_reference_extraction_ocr_is_disabled(tmp_path: Path) -> None:
    image_path = tmp_path / "question.png"
    make_image(image_path)
    extractor = LocalReferenceExtractor()

    with pytest.raises(LocalReferenceExtractionError, match="PaddleOCR has been removed"):
